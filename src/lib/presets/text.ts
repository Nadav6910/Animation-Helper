...
export const TEXT_PRESETS: Preset[] = [
  ...
  {
    id: 'text-rise-reveal',
    name: 'Letters · rise & reveal',
    category: 'text',
    description: 'Each letter springs up from below, un-blurring into focus',
    build: () => ({
      target: 'text', selector: '.animated', shape: 'square', text: 'Animate', svgPath: 'check',
      iterations: 1, direction: 'normal', fill: 'both',
      // Inspired by the splash screen's title reveal: each letter
      // starts invisible, blurred, and translated 24 px down, then
      // springs up + un-blurs + fades in. The combination of three
      // simultaneous animated properties (opacity / blur / translate)
      // gives the cycle real motion throughout, which means it loops
      // cleanly under iterations: infinite — every letter cycles
      // smoothly between its start and end state rather than
      // snapping at one instant the way the previous step-eased
      // typewriter did.
      //
      // 35 ms stagger matches the splash screen's letter cadence —
      // tight enough that the reveal feels orchestrated, not
      // mechanical. easeOutQuint approximates the spring landing
      // without overshoot.
      keyframes: [
        { id: uid(), at: 0, opacity: 0, blur: 10, transform: { ...blank(), translate: [0, 24] } },
        { id: uid(), at: 100, opacity: 1, blur: 0, transform: { ...blank() } },
      ],
